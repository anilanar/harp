...
describe('Cacher', () => {
    ...
    describe('unkeyed', () => {
        ...
        it('should not throw when marking a undefined fragment', () => {
            c.putChild(cache, undefined);
        ...
        it('should unmount multiple fragments', () => {
            const fragments = times(5).map(i => ({i}));
            fragments.forEach(f => c.putChild(cache, f));
